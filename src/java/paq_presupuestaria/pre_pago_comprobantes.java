...
//        agregarComponente(quinde);
    ...
    public void aceptarBusqueda() {
        ...
        if (set_comprobante.getValorSeleccionado() != null) {
            TablaGenerica tabDatos = programas.Beneficiarios(Integer.parseInt(set_comprobante.getValorSeleccionado()));
            if (!tabDatos.isEmpty()) {
...
//        tdd.getMenuTabla().getItem_actualizar().setRendered(false);//nucontextual().setrendered(false);
    ...
    public void save_listado() {
        for (int i = 0; i < tab_detalle1.getTotalFilas(); i++) {
            programas.numTransComprobante(txt_num_listado.getValue() + "", cal_fecha.getFecha(), tab_detalle1.getValor(i, "comprobante"), Integer.parseInt(tab_detalle1.getValor(i, "ide_listado")), Integer.parseInt(tab_detalle1.getValor(i, "ide_detalle_listado")));
        }
